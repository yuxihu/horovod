    ...
    def _allreduce_grads(self):
        for i, param in enumerate(self._params):
            if param.grad_req != 'null':
                grad_compressed, dtype = self._compression.compress(param.list_grad()[0])
                allreduce_(grad_compressed, average=False,
                           name=str(i), priority=-i)
                param.list_grad()[0] = self._compression.decompress(grad_compressed, dtype)
